  ...
  const fetchAllData = async () => {
    try {
      ...
      if (data.options) {
        ...
        // AI 自动招呼语开关：后端为 0/1，前端直接回显为数字
        const normalizedEnableAi = (data.config?.enableAi ?? 0)
        setConfig(prev => ({ ...prev, enableAi: normalizedEnableAi }))

        // 发送图片简历开关：后端为 0/1，前端直接回显为数字
        const normalizedSendImgResume = (data.config?.sendImgResume ?? 0)
        setConfig(prev => ({ ...prev, sendImgResume: normalizedSendImgResume }))
        ...
        <TabsContent value="config" className="space-y-6 mt-6">
          ...
          <Card className="animate-in fade-in slide-in-from-bottom-5 duration-700">
            ...
            <CardContent>
              ...
              {/* AI 自动招呼语开关 */}
              <div className="space-y-2">
                <Label htmlFor="enableAi">AI 自动招呼语</Label>
                <Select
                  id="enableAi"
                ...
                <p className="text-xs text-muted-foreground">开启后将读取您的技能介绍结合岗位 JD 追发个性化招呼语。</p>
              </div>

              {/* 发送图片简历开关 */}
              <div className="space-y-2">
                <Label htmlFor="sendImgResume">发送图片简历</Label>
                <Select
                  id="sendImgResume"
                ...
                <p className="text-xs text-muted-foreground">是否在直投拉起沟通后，自动向对方发送图片格式简历。</p>
              </div>
              </div>
            </CardContent>
        </Card>
