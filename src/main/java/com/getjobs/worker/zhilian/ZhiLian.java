...
public class ZhiLian {
    ...
    private boolean deliverCurrentPage(String keyword) {
        // 点击前定义并注册统一的子窗口关闭监听器（整个方法期内均有效，防异步遗漏）
        java.util.function.Consumer<Page> closer = (Page newPage) -> {
            try {
                if (newPage.opener() == page) {
                    ...
                    try { PlaywrightUtil.sleepMillis(300); } catch (Exception ignored) {}
        ...
        try {
            ...
            for (PageJob pj : jobs) {
                ...
                // 检查是否已经投递过或者是置灰不可用状态，防范 Playwright 的 30 秒 Actionability Checks 等待卡死
                boolean isBtnDisabled = false;
                try {
                    isBtnDisabled = applyBtn.isDisabled();
                } catch (Exception ignored) {}
                ...
                if (isBtnDisabled || (btnText != null && btnText.contains("已投递"))) {
                    log.info("岗位【{}】按钮为禁用或已投递状态，直接跳过", pj.jobTitle);
                    try {
