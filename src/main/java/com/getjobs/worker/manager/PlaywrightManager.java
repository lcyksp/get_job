  ...
  private static final String JOB51_URL = "https://www.51job.com";
    ...
    public void init() {
        ...
        try {
            ...
            // 使用持久化用户配置目录启动浏览器，从而持久化登录状态并保存 Cookie
            java.nio.file.Path userDataDir = java.nio.file.Paths.get("chrome_user_data");
            context = playwright.chromium().launchPersistentContext(userDataDir, 
                    new BrowserType.LaunchPersistentContextOptions()
                            .setHeadless(false) // 可视化调试
                            .setSlowMo(50)      // 放慢操作速度，便于调试
                            .setViewportSize(null) // 使用浏览器窗口实际大小
            ...
            log.info("✓ Chrome 浏览器已使用持久化用户目录 [{}] 启动 (调试端口: {})", userDataDir.toAbsolutePath(), CDP_PORT);
